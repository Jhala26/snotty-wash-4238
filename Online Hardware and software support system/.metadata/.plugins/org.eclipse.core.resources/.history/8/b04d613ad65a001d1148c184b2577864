...
//	Override  Method defination of EngineerAllotComplain() Interface method


	@Override
	public List<ComplainStatus> EngineerAllotComplain(int engId) throws EngineerException {
		
		List<ComplainStatus> compList = new ArrayList<>();
			...
			PreparedStatement ps = conn.prepareStatement("select c.compId, c.comType, c.compStatus, e.engId, e.engName, e.EngUsername"
					+ " from complain c INNER JOIN engineer e INNER JOIN complain_engineer ce"
					+ " ON c.compID = ce.compId AND e.engId = ce.engID AND e.engId = ?");
			...
			if(rs.next()){
				...
				int compId = rs.getInt("compId");
				int engIds = rs.getInt("engId");
				String engName = rs.getString("engName");
				String engEmail = rs.getString("EngUsername");
				String comType = rs.getString("comType");
				String compStatus = rs.getString("compStatus");
				
				ComplainStatus comp = new ComplainStatus(compId, engIds, engName, engEmail, comType, compStatus);
				
				compList.add(comp);
				
			}
			else {
				throw new EngineerException("Invalid Engineer Id............");
			}
...
//	Override  Method defination of engineerUpdateStatus() Interface method


	@Override
	public String engineerUpdateStatus(int compId, String newStatus) throws ComplainException {
		...
		try (Connection conn = DBUtil.provideConnection()){
			...
			PreparedStatement ps = conn.prepareStatement("select * from complain where compId = ?");
			...
			if(rs.next()) {
				...
				PreparedStatement ps2 = conn.prepareStatement("update complain set CompStatus = ? where compId = ?");
...
//	Override  Method defination of complainAttended() Interface method


	@Override
	public List<ComplainStatus> complainAttended(int engId) throws EngineerException {
		
		List<ComplainStatus> attend = new ArrayList<>();
		...
		try (Connection conn = DBUtil.provideConnection()){
			...
			PreparedStatement ps = conn.prepareStatement("select c.compId, c.comType, c.compStatus, e.engId, e.engName, e.EngUsername"
					+ " from complain c INNER JOIN engineer e INNER JOIN complain_engineer ce"
					+ " ON c.compID = ce.compId AND e.engId = ce.engID AND e.engId = ? AND c.compStatus = ?");
...
//	Override  Method defination of changeEngineerPassword() Interface method

	@Override
	public String changeEngineerPassword(int engId, String oldPass, String newPass) throws EngineerException {
		...
		try (Connection conn = DBUtil.provideConnection()){
			...
			PreparedStatement ps = conn.prepareStatement("select * from engineer where engID = ? AND engPassword = ? ");
			...
			if(rs.next()) {
				...
				PreparedStatement ps2 = conn.prepareStatement("update engineer set engPassword = ? where engID = ?");
				ps2.setString(1, newPass);
				ps2.setInt(2, engId);
				...
				throw new EngineerException("Invalid Engineer ID or Old Password....");
